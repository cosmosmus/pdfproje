...
export async function POST(
  ...
  if (file.size > MAX_PDF_SIZE_BYTES) {
    return NextResponse.json({ error: "PDF 50MB'tan küçük olmalı" }, { status: 400 });
  }
  ...
  return NextResponse.json({ ok: true, pageCount: updated.pageCount, updatedAt: updated.updatedAt });
}
